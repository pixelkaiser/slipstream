...
use warpui::elements::{
    Align, CacheOption, ChildAnchor, ChildView, ClippedScrollStateHandle, ConstrainedBox,
    Container, CornerRadius, CrossAxisAlignment, Dismiss, Fill, Flex, FormattedTextElement,
...
impl LoginSlideView {
    ...
    fn should_render_provider_setup(&self) -> bool {
        ChannelState::product_name() == "Slipstream"
            && self.ai_enabled
            && matches!(self.source, LoginSlideSource::OnboardingFlow)
    }
    ...
    fn render_select_auth_content(
        ...
        } else if matches!(self.login_purpose(), LoginPurpose::WarpDrive) {
            format!("Get started with {product_name} Drive")
        } else if matches!(self.login_purpose(), LoginPurpose::ThirdParty) {
        ...
        let subtitle_text = if provider_setup {
            ...
            format!(
                "Create a {product_name} account to enable AI-powered planning, coding, and automations."
            )
        } else {
        ...
        let (title, body, features, cancel_label): (
                ...
                if ChannelState::product_name() == "Slipstream" =>
            {
                (
                    ...
                    "Slipstream is better with AI. By continuing, you won't have access to any of the following features:",
                    ai_features(),
                    "Enable AI features",
                )
            }
