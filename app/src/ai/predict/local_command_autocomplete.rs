...
pub async fn request_local_command_autocomplete(
    ...
    if let Some(local_max_completion_tokens) = provider_settings.local_max_completion_tokens {
        request_builder = request_builder.header(
            LOCAL_MAX_COMPLETION_TOKENS_HEADER,
            local_max_completion_tokens.to_string(),
        );
    }
    if let Some(local_model_context_tokens) =
        provider_settings.local_model_context_tokens.as_deref()
    {
        request_builder = request_builder.header(
