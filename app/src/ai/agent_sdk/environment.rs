            ...
            Ok(response) => match response.list_warp_dev_images {
                ListWarpDevImagesResult::ListWarpDevImagesOutput(output) => {
                    ...
                    if matches!(
                        ...
                        println!(
                            "All default dev images contain Python and Node. For more information, see: {}\n",
        ...
        ctx.spawn(initial_sync, move |_, result, ctx| {
            if result.is_err() {
                super::report_fatal_error(
                    anyhow::anyhow!("Timed out waiting for Drive to sync"),
                    ctx,
                );
                return;
            }
            ...
            Ok(response) => match response.list_warp_dev_images {
                ListWarpDevImagesResult::ListWarpDevImagesOutput(output) => {
                    if output.images.is_empty() {
                        super::report_fatal_error(
                            anyhow::anyhow!("No default dev images available."),
                            ctx,
                        );
                        return;
                    }
