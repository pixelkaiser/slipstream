...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    toggle_binding_pairs.push(
        ToggleSettingActionPair::new(
            "cursor trail",
            builder(SettingsAction::AppearancePageToggle(
                AppearancePageAction::ToggleCursorTrail,
            )),
            context,
            flags::CURSOR_TRAIL_CONTEXT_FLAG,
        )
        .is_supported_on_current_platform(
    ...
    // Add command palette entry for toggling between modern and classic input modes
    app.register_fixed_bindings(vec![FixedBinding::empty(
        "Toggle Input Mode (Modern/Classic)".to_string(),
        builder(SettingsAction::AppearancePageToggle(
...
impl TypedActionView for AppearanceSettingsPageView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            ToggleCursorTrail => self.toggle_cursor_trail(ctx),
...
impl AppearanceSettingsPageView {
    ...
    fn build_page(ctx: &mut ViewContext<Self>) -> PageType<Self> {
        ...
        categories.push(Category::new(
            ...
            vec![
                ...
                Box::new(CursorTrailWidget::default()),
            ],
        ));
    ...
    fn input_mode_dropdown_item_label(val: InputMode) -> &'static str {
        match val {
            InputMode::PinnedToBottom => "Pin to the bottom (Modern mode)",
...
impl SettingsWidget for CustomAppIconWidget {
    ...
    fn render(
        ...
        let show_dock_icon_toggle = render_body_item::<AppearancePageAction>(
            format!("Show {} in Dock", ChannelState::product_name()),
            None,
            LocalOnlyIconState::for_setting(
    ...
    fn render(
        ...
        let radio_buttons = appearance
            ...
            .radio_buttons(
                ...
                vec![
                    RadioButtonItem::text(ChannelState::product_name()),
...
#[derive(Default)]
struct CursorTrailWidget {
    switch_state: SwitchStateHandle,
}

impl SettingsWidget for CursorTrailWidget {
    type View = AppearanceSettingsPageView;
    ...
    fn render(
        ...
        render_body_item::<AppearancePageAction>(
            "Cursor trail (experimental)".into(),
            None,
            LocalOnlyIconState::for_setting(
                CursorTrailEnabled::storage_key(),
                CursorTrailEnabled::sync_to_cloud(),
                &mut view.local_only_icon_tooltip_states.borrow_mut(),
                app,
            ),
            ToggleState::Enabled,
            ...
            appearance
                .ui_builder()
                .switch(self.switch_state.clone())
                .check(*terminal_settings.cursor_trail_enabled.value())
                .build()
                .on_click(move |ctx, _, _| {
                    ctx.dispatch_typed_action(AppearancePageAction::ToggleCursorTrail);
                })
                .finish(),
            None,
        )
