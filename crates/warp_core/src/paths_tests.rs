...
fn test_data_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(data_dir(), home_dir.join(".slipstream"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(data_dir(), home_dir.join(".local/share/slipstream"));
        } else if #[cfg(windows)] {
            assert_eq!(data_dir(), home_dir.join("AppData\\Roaming\\slipstream\\Slipstream\\data"));
        } else {
...
fn test_config_local_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(config_local_dir(), home_dir.join(".slipstream"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(config_local_dir(), home_dir.join(".config/slipstream"));
        } else if #[cfg(windows)] {
            assert_eq!(config_local_dir(), home_dir.join("AppData\\Local\\slipstream\\Slipstream\\config"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_warp_home_config_dir_path() {
    ...
    let expected_dir_name = match ChannelState::data_profile() {
        Some(data_profile) => format!(".slipstream-{data_profile}"),
        None => ".slipstream".to_string(),
    };
...
fn test_cache_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(cache_dir(), home_dir.join("Library/Application Support/com.slipstream.app"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(cache_dir(), home_dir.join(".cache/slipstream"));
        } else if #[cfg(windows)] {
            assert_eq!(cache_dir(), home_dir.join("AppData\\Local\\slipstream\\Slipstream\\cache"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_state_dir_path() {
    ...
    cfg_if::cfg_if! {
        // ChannelState, by default, is configured for Channel::Oss.
        if #[cfg(target_os = "macos")] {
            assert_eq!(state_dir(), home_dir.join("Library/Application Support/com.slipstream.app"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(state_dir(), home_dir.join(".local/state/slipstream"));
        } else if #[cfg(windows)] {
            assert_eq!(state_dir(), home_dir.join("AppData\\Local\\slipstream\\Slipstream\\data"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_project_path_for_oss_app_id() {
    let project_dirs = project_dirs_for_app_id(AppId::new("com", "slipstream", "app"), None)
        .expect("should be able to compute project dirs");
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(project_dirs.project_path(), "com.slipstream.app");
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(project_dirs.project_path(), "slipstream");
        } else if #[cfg(windows)] {
            assert_eq!(project_dirs.project_path(), "slipstream\\Slipstream");
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
